export class DishonoredTracker extends Application {
	...
	activateListeners(html) {
		...
		// windows, then consistently check for updates, this should be seperate
		// as we want the check function to be asyncherous.
		renderTracker();
		this.checkUpdates();

		// Hide the increase and decrease buttons if the user doesn't have
		// permissions.
		...
		// Find the button which has the class #dishonored-chaos-track-increase
		// and tie the following to the onClick
		html.find("#dishonored-chaos-track-increase").click(function() {
			// Check if the player has permissions before moving on!
			if (game.user.hasRole(game.settings.get("FVTT-Dishonored", "chaosPermissionLevel"))) {
				// Set momentum, we do this as otherwise when we render the
				// tracker it will use the old initial load momentum (THANKS mkscho63)
				momentum = game.settings.get("FVTT-Dishonored", "momentum");
				chaos = parseInt(document.getElementById("dishonored-track-chaos").value);
				// Check if chaos is currently 99, anything past this is unable
				// to render. If it does error it becomes a meme.
				if (chaos === 99) {
		...
		html.find("#dishonored-chaos-track-decrease").click(function() {
			...
			if (game.user.hasRole(game.settings.get("FVTT-Dishonored", "chaosPermissionLevel"))) {
				// Set momentum, we do this as otherwise when we render the
				// tracker it will use the old initial load momentum
				// (THANKS mkscho63)
				momentum = game.settings.get("FVTT-Dishonored", "momentum");
				chaos = parseInt(document.getElementById("dishonored-track-chaos").value);
				// check if chaos is currently 0, if so it cannot be decreased
				// so error.
				if (chaos === 0) {
		...
		html.find("#dishonored-track-chaos").change(function() {
			// Check if the player has permissions before moving on!
			if (game.user.hasRole(game.settings.get("FVTT-Dishonored", "chaosPermissionLevel"))) {
				// Set chaos and momentum, we do this as otherwise when we
				// render the tracker it will use the old initial load chaos
				// (THANKS mkscho63)
				momentum = game.settings.get("FVTT-Dishonored", "momentum");
				...
				if (isNaN(document.getElementById("dishonored-track-chaos").value) && document.getElementById("dishonored-track-chaos").value.substr(0, 1) !== "+" && document.getElementById("dishonored-track-chaos").value.substr(0, 1) !== "-") {
					...
					if (document.getElementById("dishonored-track-chaos").value.substr(0, 1) === "+") {
						// As we allow any + value through we want to check the
						// rest is actually a number, this is caught here.
	...
	async checkUpdates() {
		...
		setInterval(function() {
			// Define Chaos and Momentum as the value displayed and the
			// storedChaos and Momentum as the value saved to the settings
			...
			// Get out clause that checks if the chaos or momentum is below 0
			// and if so, change it to 0.
			if (storedChaos < 0) {
			...
			// Check if the field is currently focused, we don't want to
			// interupt the user if they have the field focused!
			if ($("#dishonored-track-chaos").is(":focus") === false) {
				// If the displayed and stored value is not equal, make the
				// displayed value equal the stored value!
