...
export class DishonoredNPCSheet extends ActorSheet {
	...
	activateListeners(html) {
		...
		// a child called "bar-stress-renderer".
		let stressTrackUpdate = function() {
			...
			// This checks that the max-stress hidden field is equal to the
			// calculated Max Stress value, if not it makes it so.
		...
		html.find(".control.toggle").click(ev => {
			...
			if (item.system.equipped === false) {
				...
				else if (itemType === "armor" && !isHelmet && armorNumber >= 1) {
					ui.notifications.error(game.i18n.localize("dishonored.notifications.armorAlreadyEquipped"));
					return false;
				}
				else if (itemType === "armor" && isHelmet && helmetNumber >= 1) {
		...
		// each scenario. Stress uses the same logic.
